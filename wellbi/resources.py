...
def clinics():
    clinics = [['Castro-Mission Health Center', {'lat': 37.7631636584137, 'lng': -122.43202042310821}],
    ...
    if form.validate_on_submit():
        ...
        closest = {}
        for i in range(len(clinics)): #carry out calculations to actually find closest clinics
            clinic_coords = (clinics[i][1]['lat'], clinics[i][1]['lng'])
            distance = geopy.distance.geodesic(user_coords, clinic_coords).miles
            closest[clinics[i][0]]= distance
        closest = heapq.nsmallest(5,closest,key=lambda x: (closest.get(x),x))
        return render_template("resources.html", form=form, zipcode=zip, clinics=closest)
